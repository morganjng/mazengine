...
int tile_game::tick(std::vector<mazengine::button> *presses,
                    std::vector<mazengine::button> *releases) {
    std::vector<mazengine::entity> *objs = current_map->get_objects();
    std::vector<void (*)(mazengine::button)> funcs;
    player->tick(presses, releases);
    for (mazengine::button press : *presses) {
        if (press == mazengine::KILL) {
            return 1;
        }
        for (mazengine::entity obj : *objs) {
    ...
    for (mazengine::button release : *releases) {
        for (mazengine::entity obj : *objs) {
